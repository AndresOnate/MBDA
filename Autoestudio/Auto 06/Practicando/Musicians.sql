...
DECLARE
    VECES NUMBER;
BEGIN
    SELECT COUNT(*) INTO VECES FROM band WHERE band_no = :NEW.band_id;
    IF (VECES > 1 ) THEN
...
--TuplasOk
--El nombre de la banda no debe incluir el tipo de la banda
INSERT INTO band VALUES('5', 'Swinging strings', 4, 'classical', null, 18);
--TuplasNoOK
--El nombre de la banda no debe incluir el tipo de la banda
INSERT INTO band VALUES('6', 'The Rest classical', 4, 'classical', null, 17);
...
--AtributosOk
-- El atruibuto band_type es sólo una palabra
INSERT INTO band VALUES('1', 'AASO', 6, 'classica', null, 10);
--- TBandName El atributo band Name debe terner almenos uan letra
INSERT INTO band VALUES('2', 'BBSO', 5, 'classica', null, 11);

--AtributosNoOk
-- El atruibuto band_type es sólo una palabra
INSERT INTO band VALUES('3', 'AASO', 6, 'clas V', null, 9);
-- Dos palabras
--- TBandName El atributo band Name debe terner almenos una letra
INSERT INTO band VALUES('4', '', 5, 'classica', null, 8);
-- band_name de longitud cero


--Acciones
--La banda se puede eliminar en cualquier momento, si no ha participado en conciertos.
ALTER TABLE performance DROP CONSTRAINT FK_PERFORMANCE_GAVE;
ALTER TABLE performance ADD CONSTRAINT FK_PERFORMANCE_GAVE FOREIGN KEY (gave) REFERENCES band(band_no);
--AccionesOk
...
--- Consultando
--1. El nombre y dirección del sitio web de todas las bandas.
SELECT band_name, EXTRACTVALUE(detail,'/Detalle/WebSite/text()') AS URL
FROM band;

--2. El nombre de las bandas que tienen logos de color rojo
SELECT band_name
FROM band
WHERE  EXTRACTVALUE(detail,'/Detalle/Logo/@color') = 'Rojo';

--3.  Las influencias de una banda dada
SELECT band_name, EXTRACTVALUE(detail,'/Detalle/Influencias/Influencia[1]/@nombre') AS Influencia1, EXTRACTVALUE(detail,'/Detalle/Influencias/Influencia[2]/@nombre') AS Influencia2,  EXTRACTVALUE(detail,'/Detalle/Influencias/Influencia[3]/@nombre')  AS Influencia3
FROM band
WHERE band_name = 'ROP';

--4.  La banda que ha logrado mayores ventas en sus discos
SELECT band_name, MAX(EXTRACTVALUE(detail,'/Detalle/Discografia/Discos/Disco[1]/@ventas') + EXTRACTVALUE(detail,'/Detalle/Discografia/Discos/Disco[2]/@ventas')) AS TOTAL
FROM band
GROUP BY band_name
ORDER BY TOTAL DESC;
...
SELECT band_name, EXTRACTVALUE(detail,'/Detalle/PremiosYNominaciones/Premio/@trabajo') AS URL
FROM band
WHERE EXTRACTVALUE(detail,'/Detalle/PremiosYNominaciones/Premio/@año') = '2011'
;
...
<?xml version="1.0"?>
<!DOCTYPE Detalle[
    <!ELEMENT Detalle(WebSite+ | Logo | Influencias | Discografia | PremiosYNominaciones)*>
    <!ELEMENT WebSite (#PCDATA)>
    <!ELEMENT Logo EMPTY>
    <!ELEMENT Influencias (#PCDATA)>
    <!ELEMENT Discografia (Discos)>
    <!ELEMENT Discos (Disco)>
    <!ELEMENT Disco EMPTY>
    <!ELEMENT PremiosYNominaciones(Premio)>
    <!ELEMENT Premio EMPTY>
    <!ATTLIST Logo color CDATA #REQUIRED>
    <!ATTLIST Logo texto CDATA #REQUIRED)>
    <!ATTLIST Logo imagen CDATA #IMPLIED>  
    <!ATTLIST Disco nombre CDATA #REQUIRED>
    <!ATTLIST Disco año CDATA #REQUIRED>
    <!ATTLIST Disco venta CDATA #IMPLIED>  
    <!ATTLIST Premio año CDATA #REQUIRED>
    <!ATTLIST Premio trabajo CDATA #REQUIRED>
    <!ATTLIST Premio nombre (CDATA #REQUIRED>
    <!ATTLIST Premio resultado CDATA #REQUIRED>    
]>
