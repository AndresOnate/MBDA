...
END;
/ 
CREATE OR REPLACE TRIGGER TR_CATEGORIAS_BD
    BEFORE DELETE ON categorias
    FOR EACH ROW
    BEGIN
        RAISE_APPLICATION_ERROR(-20135, 'No se puede eliminar una categoria');
END;
/
CREATE OR REPLACE TRIGGER TR_CATEGORIAS_BD
    BEFORE DELETE ON categorias
...
CREATE OR REPLACE TRIGGER TR_PLATAFORMAS_BD
    BEFORE DELETE ON plataformas
    FOR EACH ROW
    BEGIN
        RAISE_APPLICATION_ERROR(-20136, 'No se puede eliminar una plataforma');
END;
/ 
